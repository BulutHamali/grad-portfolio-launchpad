...
# GATK4 Germline Variant Calling Pipeline - Chromosome 17
...
# Modified for chromosome 17 analysis
...
samtools faidx hg38.fa chr17 > chr17.fa # Extracting the chromosome 17 from the reference genome
samtools faidx chr17.fa # Indexing the chromosome 17 reference genome

gatk CreateSequenceDictionary R=chr17.fa O=chr17.dict # Creating a sequence dictionary for the reference genome
...
bwa index chr17.fa # Indexing the reference genome
...
for file in H*.bam; do
  ...
  bwa mem -t 7 -M -R "$read_group" chr17.fa $fastq | samtools view -bS - | samtools sort -o "aligned${samplename}.bam"
...
echo "Processing chr17_cohort.vcf"
stats=$(bcftools stats ../vcf/chr17_cohort.vcf | grep "^SN" | head -n 9 | awk -F':' '{printf("%s,", $2)}' | sed 's/,$//')
echo "chr17_cohort.vcf,$stats" >> "$cohort_stats_file"
echo "Metrics for chr17_cohort.vcf saved to $cohort_stats_file"
...
gatk ApplyVQSR \
  -R chr17.fa \
  -V ../vcf/chr17_cohort.vcf \
  -O ../vcf/SNP_filtered_chr17_cohort.vcf \
  --tranches-file ../statistics/SNP.tranches \
...
gatk ApplyVQSR \
  -R chr17.fa \
  -V ../vcf/SNP_filtered_chr17_cohort.vcf \
  -O ../vcf/recal_filtered_chr17_cohort.vcf \
  --tranches-file ../statistics/INDEL.tranches \
...
bcftools view --samples-file relevant_samples.txt ../vcf/recal_filtered_chr17_cohort.vcf > ../vcf/relevant_recal_filtered_chr17_cohort.vcf

#-------------------------------------Variant Annotation--------------------------------------
# Running snpEff for variant annotation
echo "Annotating filtered VCF with snpEff"
java -jar ../../../apps/snpEff/snpEff.jar -v -stats ../statistics/annot_metrics.html -csvStats ../statistics/annot_metrics.csv hg38 ../vcf/relevant_recal_filtered_chr17_cohort.vcf > ../vcf/annotated_relevant_recal_filtered_chr17_cohort.ann.vcf

echo "=== Chromosome 17 Variant Calling Pipeline Complete ==="
echo "Final annotated VCF: ../vcf/annotated_relevant_recal_filtered_chr17_cohort.ann.vcf"
